...
export async function getFtpClient(data: FormData) {
  ...
  // Sentinel Security Fix: Use env password if placeholder is provided and host matches
  ...
  if (password === '********') {
    if (host === envHost) {
      ...
      if (envPassword) {
        ...
        logger.info('Using environment password for FTP connection.');
      } else {
        logger.warn('Placeholder password provided but no environment password found.');
      }
    } else {
      logger.warn('Placeholder password provided but host does not match environment host. Authentication will likely fail.');
    }
  }
