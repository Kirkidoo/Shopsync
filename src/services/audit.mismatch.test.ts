...
describe('findMismatches', () => {
    ...
    describe('Clearance Logic', () => {
        ...
        it('flags incorrect template if tag is present but template is wrong', () => {
            ...
            const result = findMismatches(csv, shopify, filename);
            expect(result).toContainEqual(expect.objectContaining({ field: 'incorrect_template_suffix', csvValue: 'clearance' }));
        });
        ...
        it('ignores clearance checks if price equals compareAtPrice (Exception)', () => {
            ...
            expect(result).not.toContainEqual(expect.objectContaining({ field: 'missing_clearance_tag' }));
            expect(result).not.toContainEqual(expect.objectContaining({ field: 'incorrect_template_suffix' }));
        });
    });
    ...
    describe('Oversize Logic', () => {
        ...
        it('OVERRIDES Clearance logic: Oversize product in Clearance file uses heavy-products', () => {
            ...
            const shopify = createProduct({ tags: 'oversize, clearance', templateSuffix: 'heavy-products' });
