...
export function AuditTable({
  ...
  handleMarkAsCreated,
  handleCreate,
  handleOpenMissingVariantMediaManager,
  handleBulkCreateVariants,
  setEditingMediaFor,
  setEditingMissingMedia,
  handleFixSingleMismatch,
  handleMarkAsFixed,
  handleDeleteVariant,
  handleDeleteProduct,
  statusConfig,
  MISMATCH_FILTER_TYPES,
  setFixDialogHandles,
  setShowFixDialog,
  onSelectAllPage,
  isAllPageSelected,
      ...
      {(filter === 'mismatched' || filter === 'missing_in_shopify' || filter === 'all') && (
        <div className="sticky top-0 z-10 flex items-center border-b bg-background/95 px-0 py-2 backdrop-blur supports-[backdrop-filter]:bg-background/60">
          <div className="flex w-full items-center gap-4 px-3">
            ...
            <div className="flex-grow pl-3 text-xs font-medium uppercase tracking-wider text-muted-foreground">
              Product / Issue
            </div>
      ...
      <Accordion type="single" collapsible className="w-full">
        {paginatedHandleKeys.map((handle) => {
          ...
          const hasMismatch = items.some(
            (i) => i.status === 'mismatched' && i.mismatches.length > 0
          );
                      ...
                      <AlertDialogContent>
                        <AlertDialogHeader>
                          ...
                          <AlertDialogDescription>
                            ...
                            {imageCount! - items.length} unlinked images from Shopify. This cannot
                            be undone.
                          </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
                  ...
                  {productId && !isMissingVariantCase && (
                    ...
                    >
                      ...
                      {imageCount !== undefined && (
                        <span
                          className={cn(
                            imageCount > items.length ? 'font-bold text-yellow-400' : ''
                          )}
                        >
                          ({imageCount})
                        </span>
                            ...
                            <div>
                              ...
                              {item.status === 'missing_in_shopify' && (
                                <p className="text-sm text-muted-foreground">
                                  ...
                                  .
                                  {item.mismatches.some(
                                    (m) => m.field === 'heavy_product_flag'
                                  ) && (
                          ...
                          <AlertDialog>
                            ...
                            <AlertDialogContent>
                              <AlertDialogHeader>
                                ...
                                <AlertDialogDescription>
                                  All variants for &quot;{productTitle}&quot; are not in the CSV.
                                  This will permanently delete the entire product and its{' '}
                                  {items.length} variants from Shopify. This action cannot be
                                  undone.
                                </AlertDialogDescription>
                              </AlertDialogHeader>
                              <AlertDialogFooter>
