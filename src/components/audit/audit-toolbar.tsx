...
export function AuditToolbar({
  ...
  setFilterSingleSku,
  mismatchFilters,
  handleMismatchFilterChange,
  handleClearAuditMemory,
  selectedVendor,
  setSelectedVendor,
  uniqueVendors,
  isFixing,
      ...
      <div className="flex flex-col gap-4 rounded-lg border bg-card p-4 shadow-sm md:flex-row md:items-center">
        ...
        <Popover>
          ...
          <PopoverContent className="w-60 p-0" align="end">
            <div className="grid max-h-[300px] gap-1 overflow-y-auto p-2">
      ...
      {Object.keys(columnFilters).length > 0 && (
        <div className="flex flex-wrap gap-2 animate-in fade-in slide-in-from-top-1">
          {Object.entries(columnFilters).map(([col, val]) => (
            <div
              key={col}
              className="flex w-full items-center gap-2 rounded-md border bg-muted p-2 text-sm shadow-sm md:w-auto"
            >
              <span className="font-semibold text-muted-foreground">{col}:</span>
              <Input
                className="h-7 w-32 text-xs md:w-48"
