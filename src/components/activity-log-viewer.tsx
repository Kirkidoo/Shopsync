...
// Memoized log item to prevent re-rendering identical logs when list refreshes
const LogEntryItem = memo(({ log }: { log: LogEntry }) => {
  ...
  // Custom comparison to ensure we don't re-render if the ID matches
  // We assume logs are immutable, so checking ID is sufficient.
  // We also check timestamp just in case the same ID could be reused (unlikely for logs but safe).
