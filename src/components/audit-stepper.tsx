    ...
    startTransition(async () => {
      try {
        ...
        if (!useCache) {
          ...
          result = await runBulkAuditFromDownload(
            csvProducts,
            selectedCsv,
            operation.resultUrl,
            locationId
                ...
                <div className="grid gap-3">
                  {csvFiles.map((file) => (
                    <button
                      ...
                      className={cn(
                        'relative flex w-full cursor-pointer items-center gap-4 rounded-lg border p-4 text-left transition-all hover:bg-accent/5 focus:outline-none focus-visible:ring-2 focus-visible:ring-ring',
