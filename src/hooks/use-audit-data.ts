...
export function useAuditData({ initialData, initialSummary }: UseAuditDataProps) {
  ...
  const filteredData = useMemo(() => {
    ...
    if (filter === 'mismatched' && mismatchFilters.size > 0) {
      results = results.filter((item) => item.mismatches.some((m) => mismatchFilters.has(m.field)));
    }
  ...
  // Pagination
  const handleKeys =
    filter === 'duplicate_in_shopify' ? Object.keys(groupedBySku) : Object.keys(groupedByHandle);
