...
import { markMismatchAsFixed, markProductAsCreated } from '@/lib/utils';
...
export function useAuditActions({
  ...
  const handleFixSingleMismatch = (item: AuditResult, fixType: MismatchDetail['field']) => {
    if (fixType === 'duplicate_in_shopify' || fixType === 'heavy_product_flag') {
  ...
  const handleBulkFix = (targetHandles?: Set<string>, targetTypes?: MismatchDetail['field'][]) => {
    ...
    startTransition(async () => {
      try {
        ...
        if (itemsToFix.length === 0) {
          toast({
            title: 'Info',
            description: 'No mismatches found to fix based on current selection.',
          });
          handleActionEnd();
          return;
        }
        ...
        if (result.success) {
          ...
          toast({
            title: 'Error',
            description: result.message || 'Bulk fix failed.',
            variant: 'destructive',
          });
        }
      } catch (error) {
        ...
        toast({
          title: 'Error',
          description: 'An error occurred during bulk fix.',
          variant: 'destructive',
        });
      } finally {
        handleActionEnd();
  ...
  const handleCreate = (item: AuditResult) => {
    ...
    if (!missingType) {
      toast({
        title: 'Error',
        description: 'Could not determine missing type (product vs variant).',
        variant: 'destructive',
      });
      return;
    }
    ...
    startTransition(async () => {
      try {
        ...
        for (const handle of handlesToCreate) {
          ...
          const isMissingProduct = handleItems.every((i) =>
        ...
        if (itemsToCreate.length === 0) {
          toast({
            title: 'Warning',
            description: 'No valid products found to create in selection.',
          });
          handleActionEnd();
          return;
        }
  ...
  const handleDeleteVariant = (item: AuditResult) => {
    ...
    if (!productId || !variantId) {
      toast({
        title: 'Error',
        description: 'Missing ID information for deletion.',
        variant: 'destructive',
      });
      return;
    }
    ...
    startTransition(async () => {
      try {
        ...
        if (result.success) {
          toast({ title: 'Success', description: result.message });
          setReportData((prev) =>
            prev.filter((d) => d.shopifyProducts[0]?.variantId !== variantId)
          );
        } else {
          toast({ title: 'Error', description: result.message, variant: 'destructive' });
        }
  ...
  const handleBulkDeleteUnlinked = (selectedProductIds: string[]) => {
    ...
    startTransition(async () => {
      try {
        ...
        toast({
          title: 'Error',
          description: 'Bulk delete images failed.',
          variant: 'destructive',
        });
      } finally {
        handleActionEnd();
